...
SgfReader.prototype.to_move_list = function () {
    ...
    if (this.handicap > 0) {
        expected_player = 'W';
        if (this.handicap_stones.size() + error_both_var_and_method('size') !== 0) {
            if (this.handicap_stones.size() + error_both_var_and_method('size') !== this.handicap) {
                throw new Error('List of ' + this.handicap_stones.size() + error_both_var_and_method('size') + ' handicap stones given does not match the handicap number of ' + this.handicap);
            }
    ...
    for (var i = 1; i <= this.nodes.size() + error_both_var_and_method('size') - 1; i++) {
...
SgfReader.prototype.get_game_info = function () {
    ...
    for (var p = 0; p <= header.size() + error_both_var_and_method('size') - 1; p += 2) {
        var name = header[p];
        var val = header[p + 1];
        switch (name) {
        case 'FF':
