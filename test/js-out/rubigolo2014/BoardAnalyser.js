...
Void.prototype.toString = function () {
    var s = 'void ' + this.code + ' (' + Grid.colorToChar(this.code) + '/' + this.i + ',' + this.j + '), size ' + this.size;
...
BoardAnalyser.prototype.findVoids = function () {
    ...
    for (var j = 1; j <= this.goban.length; j++) {
        for (var i = 1; i <= this.goban.length; i++) {
            var size;
            if ((size = this.filler.fillWithColor(i, j, main.EMPTY, voidCode, neighbors)) > 0) {
                this.voids.push(new Void(this, voidCode, i, j, size, neighbors));
                voidCode += 1;
                // keep all the groups
...
BoardAnalyser.prototype.findDyingGroups = function () {
    for (var g, g_array = this.allGroups, g_ndx = 0; g=g_array[g_ndx], g_ndx < g_array.length; g_ndx++) {
        ...
        if (ownedVoids === 1 && size + g.extraLives >= 3) {
            continue;
        }
        ...
        if (main.debug) {
            main.log.debug('eyes:' + g.eyes.length + ' owned_voids:' + ownedVoids + ' size-voids:' + size);
        }
    }
};
