...
Hunter.prototype.eval_move = function (i, j, level) {
    ...
    for (var eg, eg_array = stone.unique_enemies(this.color), eg_ndx = 0; eg=eg_array[eg_ndx], eg_ndx < eg_array.length; eg_ndx++) {
        ...
        if (empties.size() + error_both_var_and_method('size') === 1 && allies.size() + error_both_var_and_method('size') === 0) {
            ...
            if (enemies_around_empty.size() + error_both_var_and_method('size') !== 1 || enemies_around_empty[0] !== eg) {
                continue;
            }
    ...
    if (!snapback && empties.size() + error_both_var_and_method('size') < 2) {
        var lives = empties.size() + error_both_var_and_method('size');
    ...
    var taken = ( this.atari_is_caught(eg1, level) ? eg1.stones.size() + error_both_var_and_method('size') : 0 );
    var taken2 = ( eg2 && this.atari_is_caught(eg2, level) ? eg2.stones.size() + error_both_var_and_method('size') : 0 );
    var taken3 = ( eg3 && this.atari_is_caught(eg3, level) ? eg3.stones.size() + error_both_var_and_method('size') : 0 );
    if (taken < taken2) {
        taken = taken2;
    }
    if (taken < taken3) {
...
Hunter.prototype.atari_is_caught = function (g, level) {
    ...
    if (all_lives.size() + error_both_var_and_method('size') !== 1) {
        throw new Error('Unexpected: hunter #1: ' + all_lives.size() + error_both_var_and_method('size'));
    }
...
Hunter.prototype.escaping_atari_is_caught = function (stone, level) {
    ...
    var empties = stone.empties();
    if (empties.size() + error_both_var_and_method('size') !== 2) {
        empties = g.all_lives();
    }
    if (empties.size() + error_both_var_and_method('size') !== 2) {
